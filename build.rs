...
macro_rules! add_msvc_includes_to_bindings {
  ($bindings:expr) => {
    $bindings =
      $bindings.clang_arg(format!("-IC:/Program Files (x86)/Windows Kits/8.1/Include/shared"));
...
fn compile_sdl3(sdl3_build_path: &Path, target_os: &str) -> PathBuf {
  ...
  if let Ok(toolchain) = env::var("SDL3_TOOLCHAIN") {
    ...
    // Override __FLTUSED__ to keep the _fltused symbol from getting defined in the
    // static build. This conflicts and fails to link properly when building
    // statically on Windows, likely due to COMDAT conflicts/breakage happening
    // somewhere.
    ...
    #[cfg(target_os = "linux")]
    {
      ...
      if let Ok(version) = std::process::Command::new("cc").arg("-dumpversion").output() {
        let affected =
          compare_to(std::str::from_utf8(&version.stdout).unwrap(), "10", Cmp::Ge).unwrap_or(true);
        if affected {
          cfg.cflag("-fcommon");
        }
      }
  ...
  if cfg!(feature = "static-link") {
    ...
    // Prevent SDL to provide it own "main" which cause a conflict when this crate
    // linked to C/C++ program.
...
fn compute_include_paths(fallback_path: String) -> Vec<String> {
  ...
  #[cfg(feature = "pkg-config")]
  {
    // don't print the "cargo:xxx" directives, we're just trying to get the include
    // paths here
    let pkg_config_library =
      pkg_config::Config::new().print_system_libs(false).probe("sdl3").unwrap();
  ...
  #[cfg(feature = "vcpkg")]
  {
    // don't print the "cargo:xxx" directives, we're just trying to get the include
    // paths here
    let vcpkg_library = vcpkg::Config::new().cargo_metadata(false).probe("sdl3").unwrap();
...
fn copy_library_symlink(src_path: &Path, target_path: &Path) {
  if let Ok(link_path) = fs::read_link(src_path) {
    ...
    //  * deps directory: as comment example testing doesn't pick up the library
  ...
  if target_os.contains("windows") {
    ...
    for lib_dir in lib_dirs {
      ...
      if lib_path.exists() {
        found = true;
        for entry in
          std::fs::read_dir(&lib_path).unwrap_or_else(|_| panic!("Couldn't readdir {}", lib_dir))
        {
...
fn main() {
  ...
  #[cfg(feature = "bundled")]
  {
    ...
    println!("cargo:rustc-link-search={}", sdl3_compiled_path.join("lib64").display());
    println!("cargo:rustc-link-search={}", sdl3_compiled_path.join("lib").display());
  }

  let sdl3_includes = sdl3_source_path.join("include").to_str().unwrap().to_string();
